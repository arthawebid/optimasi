...
st.set_page_config(page_title="AMPL-Database CRUD", layout="wide")
st.title("🚚 Sistem Manajemen & Navigasi Antaran TK Jimbaran")
...
# ==============================================================================
# 🗄️ MANAJEMEN DATABASE (SQLite CRUD FUNCTIONS)
# ==============================================================================
...
# Eksekusi inisialisasi skema DB
inisialisasi_database()

# ==============================================================================
# 🌐 ENGINE NAVIGASI JALAN (OSRM API) & OPTIMASI MATEMATIS (AMPL)
# ==============================================================================
...
def jalankan_optimasi_ampl(all_locations, capacities):
    ...
    try:
        ...
        st.error(f"Error AMPL Engine: {str(e)}")
        return [], 0

# Tarik data dari Database SQLite
df_master_siswa = ambil_semua_siswa()

# --- MEMBUAT TAB INTERMUKA ---
tab_navigasi, tab_manajemen = st.tabs(["🗺️ Optimasi & Navigasi", "⚙️ Manajemen Data Siswa"])

# ==============================================================================
# TAB 1: OPTIMASI & NAVIGASI
# ==============================================================================
with tab_navigasi:
    ...
    if st.sidebar.button("🚀 Hitung Jaringan Lintasan AMPL"):
        with st.spinner("AMPL sedang merumuskan matriks rute searah..."):
    ...
    with col1:
        ...
        if st.session_state['calculated'] and st.session_state['active_arcs']:
            ...
            if filtered_arcs:
                ...
                st.warning(f"Armada {armada_view} tidak dialokasikan siswa aktif.")
        else:
            st.info("Pilih presensi siswa di panel kiri, lalu klik tombol Hitung Lintasan.")

    with col2:
        st.subheader("🗺️ Peta Panduan Jalan Tikus")
            ...
            for arc in st.session_state['active_arcs']:
                if arc['vehicle'] == sel_v:
                    ...
                    if idx_ke != 0:
                        ...
                        p_color = "cadetblue" if "TEMP" in lbl else v_colors[sel_v]
...
# ==============================================================================
# TAB 2: MANAJEMEN DATA SISWA (CRUD PANEL)
# ==============================================================================
with tab_manajemen:
    st.subheader("⚙️ Panel Kontrol Administrasi Data Siswa (SQLite Master)")
    
    # 1. FORM TAMBAH / UPDATE DATA SISWA
    col_form, col_table = st.columns([1, 2])
    
    with col_form:
        st.write("### 📝 Formulir Input Siswa")
        
        # Opsi apakah ingin Tambah Baru atau Edit Data Lama
        ...
        if mode_operasi == "Ubah/Edit Data Siswa Existing":
            if not df_master_siswa.empty:
                ...
                st.warning("Database kosong. Tidak ada data yang bisa diubah.")
        ...
        input_jml = st.number_input("Jumlah Anak dalam 1 Keluarga:", min_value=1, max_value=5, value=default_jml)
            ...
            if st.button("💾 Simpan Perubahan"):
                if input_nama:
                    if mode_operasi == "Tambah Siswa Baru":
                        tambah_siswa_db(input_nama, input_lat, input_lon, input_jml, input_armada, input_tipe)
                        st.success(f"Berhasil menambahkan {input_nama} secara permanen!")
                    else:
                        ubah_siswa_db(id_siswa_edit, input_nama, input_lat, input_lon, input_jml, input_armada, input_tipe)
                        st.success(f"Berhasil memperbarui data {input_nama}!")
                    st.rerun()
                else:
        ...
        with col_btn2:
            if mode_operasi == "Ubah/Edit Data Siswa Existing" and id_siswa_edit is not None:
                if st.button("🗑️ Hapus Siswa dari DB", type="primary"):
                    hapus_siswa_db(id_siswa_edit)
                    st.success("Siswa berhasil dihapus dari database permanen.")
                    st.rerun()

    # 2. PANEL KONVERSI STATUS SISWA (TEMPORARY ➔ REGULER)
    with col_table:
        st.write("### ⚡ Akselerasi Konversi Status Siswa")
        st.caption("Klik tombol konversi di bawah untuk mengubah status siswa temporary menjadi langganan reguler bulanan secara instan.")
            ...
            for idx, r in df_only_temp.iterrows():
                ...
                with col_act:
                    if st.button(f"🔄 Jadikan Permanen/Reguler", key=f"conv_{r['id']}"):
                        ...
                        st.success(f"Status {r['nama']} resmi dinaikkan menjadi Reguler!")
                        st.rerun()
        else:
            st.success("🎉 Tidak ada data siswa Temporary saat ini. Semua siswa berstatus Reguler.")
            
        st.write("---")
        st.write("### 📊 Tabel Database Master Siswa Saat Ini")
